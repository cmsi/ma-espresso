...
if [ -f "$DATA_DIR/${PACKAGE}-${VERSION_BASE}.tar.gz" ]; then :; else
  echo "Error: $DATA_DIR/${PACKAGE}-${VERSION_BASE}.tar.gz not found"
  exit 127
fi
...
cp -p $DATA_DIR/${PACKAGE}-${VERSION_BASE}.tar.gz ${PACKAGE}_${VERSION_BASE}.orig.tar.gz
